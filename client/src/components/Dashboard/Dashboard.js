...
const Dashboard = () => {
  ...
  const overDue = invoices?.filter(
    (invoice) => (invoice.dueDate <= new Date().toISOString()) && ((invoice.status === "Unpaid") || (invoice.status === "Partial"))
  );
  ...
  useEffect(() => {
    ...
    amountDisplay = totalAmount - totalPaid;
