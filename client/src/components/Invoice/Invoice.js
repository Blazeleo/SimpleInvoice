...
const Invoice = () => {
  ...
  const handleRates = (e) => {
    if(e.target.value > 100 || e.target.value < 0) e.target.value=null;
  ...
  const handleChange = (index, e) => {
    ...
    if(e.target.name === "discount" && (e.target.value < 0 || e.target.value > 100)){e.target.value = null}
    else if(e.target.name != "itemName" && e.target.value < 0){e.target.value = null}
  ...
  useEffect(() => {
    const total = () => {
      ...
      let temp = (rates / 100) * subTotal;
      setVat(parseInt(temp.toFixed(2)));
      setTotal(overallSum);
    };
    total();
